...
import { TOKEN_HEADER } from "../control-constants";
...
const config = window.__omnigentOverlays;
if (config && config.token && !window.__omnigentOverlaysPanelLoaded) {
  window.__omnigentOverlaysPanelLoaded = true;
  void main(config);
}

async function main(cfg: NonNullable<Window["__omnigentOverlays"]>): Promise<void> {
  ...
  launcher.style.cssText = style(
    ...
    "padding:8px 15px", "border:1px solid rgba(127,127,127,.4)", "border-radius:9px",
    "background:rgba(20,20,20,.85)", "color:#e6e6e6", "font:14px/1.2 ui-sans-serif,system-ui,sans-serif",
    "cursor:pointer", "opacity:.85",
  );

  const panel = document.createElement("div");
  panel.style.cssText = style(
    "position:fixed", "right:12px", "bottom:58px", "z-index:2147483646", "display:none",
    "width:min(440px,calc(100vw - 24px))", "max-height:78vh", "overflow:auto", "padding:16px",
    "border:1px solid rgba(127,127,127,.35)", "border-radius:12px",
    "background:#141414", "color:#e6e6e6", "font:14px/1.45 ui-sans-serif,system-ui,sans-serif",
  ...
  async function render(): Promise<void> {
    ...
    try {
      ...
      view = (await response.json()) as StateView;
    } catch {
  ...
  function group(title: string, items: StateItem[]): HTMLElement {
  ...
  function row(item: StateItem): HTMLElement {
  ...
  async function setEnabled(item: StateItem, toggle: HTMLButtonElement): Promise<void> {
    ...
    button.addEventListener("click", async () => {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          const result = (await response.json()) as { activeRevision: string; changed: boolean };
  ...
  function reloadBanner(): HTMLElement {
    const banner = document.createElement("div");
    banner.style.cssText = style(
      "margin:8px 0", "padding:7px 9px", "border-radius:7px", "background:#3a2f12",
      "border:1px solid #6b571f", "display:flex", "align-items:center", "gap:8px",
    );
...
function style(...rules: string[]): string {
  return rules.join(";");
}

function toggleStyle(active: boolean): string {
  return style(
    "padding:5px 13px", "border-radius:7px", "cursor:pointer", "font:13px/1.2 ui-sans-serif,system-ui,sans-serif",
