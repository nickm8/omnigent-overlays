...
import { installStyles, onDocumentReady } from "../shared/dom";
...
(function () {
  ...
  function isKnownPalette(id: string): boolean {
  ...
  function writePalette(id: string): void {
    try {
      if (id === defaultPaletteId || !isKnownPalette(id)) {
  ...
  /** The swatch to preview for a palette, honouring the current mode. */
  function activeSwatch(palette: Palette): Swatch {
    const mode = readMode();
    const dark = mode === "dark" || (mode === "system" && prefersDark());
  ...
  function installPaletteStyles(): void {
    installStyles(
      styleId,
      `
  ...
  function buildPalette(palette: Palette, currentId: string): HTMLButtonElement {
  ...
  function buildModes(currentMode: Mode): HTMLElement {
  ...
  /**
   * Re-assert the persisted palette + mode. The app applies these itself on
   * boot, so this is normally redundant — but it makes the overlay self-
   * sufficient (correct even if injected before the app, or with no app at all)
   * and idempotent, since it only ever re-applies what storage already holds.
   */
